...
def __callGraphQL(query, variables=None):
    ...
    response = requests.post(
        app.config["GRAPHQL_API"],
        json=json,
        headers=headers,
        verify=app.config["VERIFY_FLAG"],
    )
...
def get_scenes_F(
    scene_filter, sort="updated_at", direction="DESC", per_page=100, page=1
):
    query = """query findScenes($scene_filter: SceneFilterType!, $filter: FindFilterType) {
findScenes(scene_filter: $scene_filter filter: $filter ) {
count
...
def updateScene(sceneData):
  ...
  scene_markers{
    ...
    data["movies"] = [
        {"id": x["movie"]["id"], "scene_index": x["scene_index"]}
        for x in sceneData["movies"]
    ]
...
def rewrite_image_url(scene):
    ...
    scene["paths"]["screenshot"] = (
        "/image_proxy?scene_id="
        + screenshot_url.split("/")[4]
        + "&session_id="
        + screenshot_url.split("/")[5][11:]
    )
...
def show_post(scene_id):
    ...
    scene["encodings"] = [
        ...
        {"name": "webm", "videoSources": [wmshd, wmfhd]},
        {"name": "stream", "videoSources": [vs]},
    ]
    ...
    if s["interactive"]:
        ...
        scene["fleshlight"] = [
            {
                "title": Path(s["files"][0]["basename"]).stem + ".funscript",
                "url": s["paths"]["funscript"],
            }
        ]
        ...
        if "ApiKey" in headers:
            scene["fleshlight"] = [
                {
                    "title": Path(s["files"][0]["basename"]).stem + ".funscript",
                    "url": request.url_root + "script_proxy/" + s["id"],
                }
            ]
        else:
            scene["fleshlight"] = [
...
def script_proxy(scene_id):
    s = cache["scenes"][scene_id]
    r = requests.get(
        s["paths"]["funscript"], headers=headers, verify=app.config["VERIFY_FLAG"]
    )
    return Response(r.content, content_type=r.headers["Content-Type"])
...
def show_category(filter_id):
    ...
    if filter_id == f["name"]:
        ...
        return render_template(
            ...
            isGizmovr=False,
            scenes=scenes,
            sort_methods=sort_methods.keys(),
        )
    return "error?"
...
def scene(scene_id):
    ...
    if "export_deovr" in [x["name"] for x in s["tags"]]:
        enabled = True
    return render_template(
        "scene.html", scene=s, filters=config["filters"], enabled=enabled
    )
...
def refreshCache():
    ...
    if len(cache["scenes"]) == 0:
        ...
        res = get_scenes(
            scene_filter={
                "tags": {
                    "value": [tags_cache["export_deovr"]["id"]],
                    "depth": 0,
                ...
                if s["updated_at"] != cache["image_cache"][s["id"]]["updated"]:
                    ...
                    with open(os.path.join(image_dir, s["id"]), "wb") as f:
                        ...
                        with Image.open(BytesIO(r.content)) as im:
                            ...
                            rgb_im = im.convert("RGB")
                            rgb_im.save(
                                os.path.join(image_dir, s["id"] + ".thumbnail"), "JPEG"
                            )
                            cache["scenes"][int(s["id"])]["thumb"] = "/thumb/" + str(
                                s["id"]
                            )
                    ...
                    if not os.path.exists(
                        os.path.join(image_dir, s["id"] + ".thumbnail")
                    ):
...
def images(scene_id):
    if str(scene_id) in cache["image_cache"]:
        with open(cache["image_cache"][str(scene_id)]["file"], "rb") as f:
            image = f.read()
            return Response(
                image, content_type=cache["image_cache"][str(scene_id)]["mime"]
            )
    ...
    if "submit" in request.args:
        if request.args.get("submit") == "all":
            ...
            for f in files:
                ...
                if scene_id in cache["scenes"].keys():
                    ...
                    new_scene = {
                        ...
                        "date": scene["date"],
                        "performers": [
                            {"name": x["name"]} for x in scene["performers"]
                        ],
                        "tags": [{"name": x["name"]} for x in scene["tags"]],
                        "studio": scene["studio"],
                        ...
                        "scene_markers": [
                            {
                                "title": x["title"],
                                "seconds": x["seconds"],
                                "primary_tag": {"name": x["primary_tag"]["name"]},
                            }
                            for x in scene["scene_markers"]
                        ],
                    }
                    with open(os.path.join(hsp_dir, f), "rb") as fh:
                        hsp = fh.read()
                        new_scene["hsp"] = base64.standard_b64encode(hsp).decode(
                            "ascii"
                        )
                    request_s.post(
                        "https://timestamp.trade/submit-stash", json=new_scene
                    )
        else:
            ...
            new_scene = {
                ...
                "url": scene["url"],
                "date": scene["date"],
                "performers": [{"name": x["name"]} for x in scene["performers"]],
                "tags": [{"name": x["name"]} for x in scene["tags"]],
                "studio": scene["studio"],
                "stash_ids": scene["stash_ids"],
                "scene_markers": [
...
def hsp_fetch():
    ...
    if cache["hsp_fetch_job"]["running"]:
        return render_template(
            "job.html", filters=config["filters"], job=cache["hsp_fetch_job"]
        )
        ...
        if not os.path.exists(file):
            if "stash_ids" in s:
                for sid in s["stash_ids"]:
                    cache["hsp_fetch_job"]["log"].append(
                        "querying for stash id %s " % (sid["stash_id"],)
                    )
                    res = request_s.post(
            ...
            data["library"].append(
                {
                    "name": f["name"],
                    "list": [
                        request.url_root + "heresphere/" + s["id"] for s in scenes
                    ],
                }
            )
...
def heresphere_scene(scene_id):
            ...
            if t["name"] in [""]:
                ...
                for m in s["scene_markers"]:
                    ...
                    if previous_marker is not None:
                        if (
                            t["end"] - 5000 > previous_marker["seconds"] * 1000
                            and t["end"] - 5000 < previous_marker["seconds"] * 1000
                        ):
                            found = True
                ...
                if found_marker is not None:
                    ...
                    if "id" in found_marker:
                        data = {
                            "id": found_marker["id"],
                            "title": found_marker["title"],
                            "seconds": found_marker["seconds"],
                            "scene_id": s["id"],
    ...
    if "scene_markers" in s:
        ...
        for m in s["scene_markers"]:
            if previous_marker is not None:
                tags.append(
                    {
                        "start": previous_marker["seconds"] * 1000,
                        "end": m["seconds"] * 1000,
                        "name": previous_marker["title"],
                        "track": 0,
                    }
                )
            previous_marker = m
    ...
    if s["interactive"]:
        if "ApiKey" in headers:
            scene["scripts"] = [
                {
                    "name": Path(s["files"][0]["basename"]).stem + ".funscript",
                    "url": request.url_root + "script_proxy/" + s["id"],
                    "rating": 1,
                }
            ]
